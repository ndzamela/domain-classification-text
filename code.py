...
drop_col = ['food','recharge','support','nearby','reminders','movies','casual', 'other', 'travel']
...
# store the TF-IDF vectorized data into an array
...
print(str(nb_accuracy) + ( " is the accuracy of the Multinomial Niave Bayes model"))

# Implementing SVM model (linear kernel)
lsvm = LinearSVC(random_state=0)
lsvm.fit(X_train, y_train)
y_pred = lsvm.predict(X_val)
lsvm_accuracy = accuracy_score(y_val, y_pred)
print(str(lsvm_accuracy) + ( " is the accuracy of the SVM model"))
...
# Logistic Regression model applied to the test set 
y_pred = log_reg.predict(X_test)
log_accuracy_2 = accuracy_score(y_test, y_pred)
print(str(log_accuracy_2) + ( " is the accuracy of the Logistic Regression model on the test set"))

# Multinomail NB model applied to the test set 
y_pred = nb.predict(X_test)
nb_accuracy_2 = accuracy_score(y_test, y_pred)
print(str(nb_accuracy_2) + ( " is the accuracy of the Multinomail NB model on the test set"))

# SVM  model applied to the test set 
y_pred = lsvm.predict(X_test)
lsvm_accuracy_2 = accuracy_score(y_test, y_pred)
print(str(lsvm_accuracy_2) + ( " is the accuracy of the SVM model on the test set"))
